...
// Validate Function to check blog tag length
let tagLengthChecker = (tag) => {
  // Check if blog title exists
  if (!tag) {
    return false; // Return error
  } else {
    // Check the length of tag
    if (tag.toString().length < 3 || tag.toString().length > 50) {
      return false; // Return error if not within proper length
    } else {
      return true; // Return as valid tag
    }
  }
};

// Validate Function to check if valid tag format
let tagChecker = (tag) => {
  // Check if title exists
  if (!tag) {
    return false; // Return error
...
const tagValidators = [
  // First Title Validator
  {
    validator: tagLengthChecker,
    message: 'Tag must be more than 3 characters but no more than 50'
  },
  // Second Title Validator
  {
    validator: tagChecker,
    message: 'Invaild tags'
  }
];
